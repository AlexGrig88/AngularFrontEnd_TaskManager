  ...
  public submitForm(): void {
    ...
    this.authService.updatePassword(this.passwordField.value, this.token).subscribe(
      result => { // запрос успешно выполнился без ошибок
        ...
        if (result) {
          this.router.navigate(['/info-page', {msg: 'Пароль успешно обновлен.'}]);
        }
      ...
      () => { // от сервера пришла ошибка
        ...
        this.router.navigate(['/info-page', {msg: 'Ошибка при обновлении пароля. Возможно вышел срок действия страницы. Запросите пароль заново.'}]);
